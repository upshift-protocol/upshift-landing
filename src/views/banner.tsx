...
export default function BannerView({ pools, tokens }: { pools?: IPoolWithUnderlying[]; tokens?: IToken[] }) {
  ...
  return (
    ...
    <Box>
      ...
      <Grid2 container spacing={{ xs: 4, xl: 6 }} columns={{ xs: 2, sm: 4, md: 4 }}>
        {INVESTORS_IMGS.map((inv, i) => (
        <Grid2 key={`investor-${i}`} alignItems={"center"} justifyContent={"center"} alignSelf={"center"} justifySelf={"center"}>
                <Image 
                {...inv}
          src={`/assets/investors/${inv.src}`}
        />
        </Grid2>
        ))}
      </Grid2>
